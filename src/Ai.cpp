...
void MonsterAi::update(Actor *owner) {
	...
	if(engine.map->isInFov(owner->x, owner->y)) {
		// move towards the player cuz we see them
...
ConfusedMonsterAi::ConfusedMonsterAi(int _nbTurns, Ai *_oldAi) : nbTurns(_nbTurns), oldAi(_oldAi) {}
			...
			for(Actor **iterator = engine.actors.begin(); iterator != engine.actors.end(); iterator++) {
				...
				if(actor->pickable && actor->x == owner->x && actor->y == owner->y) {
					if(actor->pickable->pick(actor, owner)) {
						found = true;
						engine.gui->message(TCODColor::lightGrey, "You picked up the %s.", actor->name);
						break;
					}
					else if(!found) {
						found = true;
