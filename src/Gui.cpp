...
void Gui::render() {
	...
	con->setDefaultBackground(TCODColor(30, 30, 30));
	con->clear();
...
void Gui::renderMouseLook() {
	if(!engine.map->isInFov(engine.mouse.cx, engine.mouse.cy)) {
		// if mouse is out of fov, do nothing
		return;
	}
...
Gui::Message::Message(const char *_text, const TCODColor &_col) : text(strdup(_text)), col(_col) {}
